...
const AuthorList = async () => {
  ...
  const authorsList = allAuthors.map(author => {
    if (author.publicProfile === "Public") {
      ...
      return <div key={author._id} className="w-full lg:w-80 flex-col justify-center items-center px-6 py-4 mb-4 border border-base-300 rounded-lg shadow-lg">
  ...
  return (
    <div className="mt-8 w-80 md:w-96 lg:w-full flex flex-col justify-center items-center lg:flex-wrap ">
      {authorsList}
    </div>
  )
}
